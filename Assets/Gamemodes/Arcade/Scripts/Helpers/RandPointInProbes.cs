...
{
    public static Vector3 Get(List<GameObject> probes)
    {
        // Set orb position
        int rand_probe_index = UnityEngine.Random.Range(0, probes.Count);

        GameObject rand_probe = probes[rand_probe_index];
        float probe_radius = rand_probe.GetComponent<ProbeProperties>().radius;
        ...
        float rand_probe_radius = UnityEngine.Random.Range(0.0f, probe_radius);
        ...
        rand_probe_point += rand_probe.transform.position;
